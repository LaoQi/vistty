...
| 2026-08-14 | 首个补丁 | 实施完成 | **000-braille.vfp**：Braille U+2800-28FF 全 256 字形入库（源 NotoMonoNerdFontMono-Regular，upem=2048）。关键能力：**gen-fontpatch 新增 `-fit-width N`（GenPatchFit）水平适配**——全局统一变换（所有字形并集 x 范围 → [0,N]），解决源字体 advance/轮廓宽度与 Sarasa cell(1024) 不匹配问题；修复 `sfnt.Segment.Args[3]` 未用槽位 {0,0} 被误当轮廓点的 bug（引入 segEach 只遍历实际槽位）。新测试：TestGenPatchFitForcesAdvance/EmptyOutline + TestBraillePatchRenders（ChainFace 端到端位图验证）。ASCII 渲染实测：2×4 点阵结构正确、无溢出、等宽对齐。回归 26 包全绿。文档 font-test.md Braille 移入已验证 + 覆盖概览/测试段 10a |
