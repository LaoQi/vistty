...
- 后端：DRM `-nogbm`（dumb buffer CPU 渲染）
...
- `master/render_loop.go`：mirror 与 independent 两路径 `Apply` 后调 `terminal.ReturnSeqPool(seqs)`
...
`blendGlyph`/`fillRect`/`memmove` 三项占 CPU ~72%，是 dumb buffer CPU 渲染固有开销（逐像素 alpha 混合 + 全屏背景清除 + backBuf→Surface 全量拷贝），需走 GBM GPU 路径（GLES 纹理上传 + `glDrawArrays` 全屏 quad）解决，`-nogbm` 下无法消除。
...
nogbm 模式无回归（帧时间保持 ~16ms，backBuf + copyAll 路径不变）。
...
6. **dumb buffer 不受益**：`-nogbm` 无 GPU，此方案仅 GBM 模式受益。dumb buffer 仍需 CPU 渲染。
