...
| Bell 响铃 | `terminal/terminal.go`（`execControl` 的 `case vte.ControlBEL`）+ `session/master.go` | OnBell 回调 + tab 标记/闪屏 + `vistty.on_bell` Lua 钩子（技术评估见 `work_docs/analysis/bell-evaluation.md`） |
...
- 光标活动保持期（Compositor.NoteActivity + cursorActivityHold=2s：用户按键/鼠标行为后光标常显不闪烁，过期回退墙钟相位；焦点 slave 生效，session 层 noteActivity 在 key/mouse 事件处触发）
