...
│   ├── options.go              # Options + OnTitle 回调 + RecordWriter
...
- ✅ 动态放大缩小（Ctrl+= 放大 / Ctrl+- 缩小 / Ctrl+0 重置，6-72pt 范围，重算行列数 + 同步 PTY winsize）
...
- `Terminal.handleScale`（terminal.go）：在 eventLoop goroutine 执行（与 Render 同线程，无并发竞争）→ faceCache.Get → SetFace → 重算 cols/rows → Resize → setPtySize → 立即 Render
- `scaleReqCh`（cap=1 非阻塞）：inputLoop 投递 → eventLoop 消费，rapid input 自动合并；select 含 `t.done` 防 goroutine 泄漏
